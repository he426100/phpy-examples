...
$model_path = getenv('MS_CACHE') . 'qwen/Qwen-1_8B-Chat';
...
[$response, $history] = PyCore::scalar($model->chat($tokenizer, "给我讲一个年轻人奋斗创业最终取得成功的故事。", history: $history));
print($response . "\n");
...
[$response, $history] = PyCore::scalar($model->chat($tokenizer, "给这个故事起一个标题", history: $history));
print($response . "\n");
# 《奋斗创业：一个年轻人的成功之路》
...
[$response] = $model->chat($tokenizer, "你好呀", history: null, system: "请用二次元可爱语气和我说话");
print($response . "\n");
# 你好啊！我是一只可爱的二次元猫咪哦，不知道你有什么问题需要我帮忙解答吗？

[$response] = $model->chat($tokenizer, "My colleague works diligently", history: null, system: "You will write beautiful compliments according to needs");
print($response . "\n");
